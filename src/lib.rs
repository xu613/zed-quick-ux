...
impl MyExtension {
    ...
    fn sync_ts_plugin(&mut self) {
        let server_version = match zed::npm_package_installed_version(PACKAGE_NAME) {
            ...
            Ok(None) => {
                println!(
                    "warning: could not determine installed {PACKAGE_NAME} version; skipping ts-plugin sync"
                );
                return;
            }
            Err(err) => {
                println!(
                    "warning: could not query {PACKAGE_NAME} version: {err}; skipping ts-plugin sync"
                );
                return;
            }
        };
